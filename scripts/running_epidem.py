...
from src.examples.example_setups import setup_stat_scm_ceo_epidem
from src.utils.sequential_sampling import sequentially_sample_model
from src.utils.sem_utils.toy_sems import EpidemiologySEM
...
def simulate_interventional_data_for_EpidemSEM(
    ...
    intervention_samples_noiseless = sequentially_sample_model(
        ...
        interventions=interv,
        sample_count=1,
        epsilon={k: np.zeros(T) for k in ["U", "T", "L", "R", "Y"]},
    )
...
init_sem, sem, true_dag_view, graphs, exploration_sets, intervention_domain, true_objective_values, ground_truth = setup_stat_scm_ceo_epidem(
    T=T,
...
#     "sem": EpidemiologySEM,  # true sem. used to generate data, which always comes from the true graph !
...
#
# Path("./results/cd_epi").mkdir(parents=True, exist_ok=True)
# Path("./results/comparison_cd_new").mkdir(parents=True, exist_ok=True)
#
# assert os.path.isdir('results/cd_epi')
# assert os.path.isdir('results/comparison_cd_new')
#
# # # # # # # # # # # # # # # # # #
# # noise = "with_noise_Epidem"
# if do_cdcbo:
#     noise = "JUST_CDCBO"
#     where = 'comparison_cd_new'
# else:
#     noise = "with_noise_Epidem"
#     where = 'cd_epi'
#
...
directory = os.getcwd() + '/results/cd_epi'
data = []
...
directory = os.getcwd() + '/results/comparison_cd_new'
...
ax.plot(best_objective_values * 40, c=np.array([1., 0.70054304, 0.37841105, 1.]), label='True optimum', lw=6,
        alpha=0.5)
ax.set_xlim((0, 35.))
ax.fill_between(range(0, len(costs['ceo']) * 2), best_objective_values[0] - 0.05 * best_objective_values[0], best_objective_values[0] + 0.05 * best_objective_values[0],
