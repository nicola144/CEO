...
from src.examples.example_setups import setup_stat_scm_ceo_toy
...
def simulate_interventional_data_for_sequential_LinearToyDBN(
    ...
    for var in interventional_data[("Z",)]:
        if len(interventional_data[("Z",)][var].shape) == 1:
            interventional_data[("Z",)][var] = interventional_data[("Z",)][var].reshape(-1, 1)
    ...
    intervention_samples_noiseless = sequentially_sample_model(
        ...
        interventions=interv,
        sample_count=1,
        epsilon={k: np.zeros(T) for k in ["X", "Z", "Y"]},
    )

    interventional_data_noiseless[("Z",)] = get_monte_carlo_expectation(intervention_samples_noiseless)
    for var in interventional_data_noiseless[("Z",)]:
        if len(interventional_data_noiseless[("Z",)][var].shape) == 1:
            interventional_data_noiseless[("Z",)][var] = interventional_data_noiseless[("Z",)][var].reshape(-1, 1)
    """
    do(X_0)
    """
    interv = make_sequential_intervention_dictionary(graph, time_series_length=T)
    ...
    for var in interventional_data[("X",)]:
        if len(interventional_data[("X",)][var].shape) == 1:
            interventional_data[("X",)][var] = interventional_data[("X",)][var].reshape(-1, 1)
    ...
    interventional_data_noiseless[("X",)] = get_monte_carlo_expectation(intervention_samples_noiseless)
    for var in interventional_data_noiseless[("X",)]:
        if len(interventional_data_noiseless[("X",)][var].shape) == 1:
            interventional_data_noiseless[("X",)][var] = interventional_data_noiseless[("X",)][var].reshape(-1, 1)
    ...
    for var in interventional_data[("X", "Z")]:
        if len(interventional_data[("X", "Z")][var].shape) == 1:
            interventional_data[("X", "Z")][var] = interventional_data[("X", "Z")][var].reshape(-1, 1)
    ...
    interventional_data_noiseless[("X", "Z")] = get_monte_carlo_expectation(intervention_samples_noiseless)
    for var in interventional_data_noiseless[("X", "Z")]:
        if len(interventional_data_noiseless[("X", "Z")][var].shape) == 1:
            interventional_data_noiseless[("X", "Z")][var] = interventional_data_noiseless[("X", "Z")][var].reshape(-1,
                                                                                                                    1)
...
init_sem, sem, true_dag_view, graphs, exploration_sets, intervention_domain, true_objective_values, ground_truth = setup_stat_scm_ceo_toy(
    T=T,
...
for s in seeds_int_data:
    current_int_data, current_int_data_noiseless = simulate_interventional_data_for_sequential_LinearToyDBN(
        graph=G,
...
CEO_input_params = {
    ...
    "sem": StatSEM,  # true sem. used to generate data, which always comes from the true graph !
...
Path("./results/comparison_cd").mkdir(parents=True, exist_ok=True)

assert os.path.isdir('results/comparison_cd')
# # # # # # # # # # # # # # # # #
noise = "with_noise_TOY"
unique_id = time.time()
with open(
        "results/comparison_cd/results_seeds_{}_n_obs_{}__nanchor_{}_ntrials_{}_ID_{}_{}_leninitial_{}.pickle".format(
