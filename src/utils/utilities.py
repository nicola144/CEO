...
def initialise_DCBO_parameters_and_objects_filtering(
        ...
        interventional_data_noiseless:dict,
        base_target: str,
        total_timesteps: int,
        task="min",
        index_name: int = None,
    ...
    # Neil: check  inputs make sense to you
    ...
    target_values_noiseless = copy.deepcopy(target_values)
    ...
    for es in exploration_sets:
        ...
        if es not in interventional_data.keys():
            ...
            interventional_samples_noiseless = interventional_data_noiseless[es]  # es on keys and nd.array on values
            ...
            # If we only have one sample per intervention we just use that
                ...
                data_subset_noiseless = interventional_samples_noiseless # TODO MODIFY ABOVE FOR NOISELESS

            # Find the corresponding target values at these coordinates [array]
            target_values[temporal_index][es] = np.array(data_subset[base_target]).reshape(-1, 1)

            target_values_noiseless[temporal_index][es] = np.array(data_subset_noiseless[base_target]).reshape(-1, 1)

            assert target_values[temporal_index][es] is not None

            # Find the corresponding interventions [array]
            if len(es) == 1:
                interventions[temporal_index][es] = np.array(data_subset[es[0]]).reshape(-1, 1)
            else:
                tmp = []
                for var in es:
                    tmp.append(data_subset[var][:, temporal_index])
                interventions[temporal_index][es] = np.vstack(tmp)
                interventions[temporal_index][es] = interventions[temporal_index][es].T
            assert interventions[temporal_index][es] is not None

            # Set the interventional data for use in DCBO
            intervention_data_Y[temporal_index][es] = target_values[temporal_index][es] #noisy data
    ...
    best_val = np.PINF if task == "min" else np.NINF
    best_es = None
    best_idx = None

    for es in target_values_noiseless[temporal_index].keys():
        # min or max for this ES
        if task == "min" and np.min(target_values_noiseless[temporal_index][es]) < best_val:
            best_val = np.min(target_values_noiseless[temporal_index][es])
            best_idx = np.argmin(target_values_noiseless[temporal_index][es])
            best_es = es
        elif task == "max" and np.max(target_values_noiseless[temporal_index][es]) > best_val:
            best_val = np.max(target_values_noiseless[temporal_index][es])
            best_idx = np.argmax(target_values_noiseless[temporal_index][es])
            best_es = es
        else:
            continue

    assert not best_es is None and not best_idx is None
    assert np.isfinite([best_val])

    # Interventions
    best_intervention_level = interventions[temporal_index][best_es][best_idx]
    # Outcomes
    best_target_value = target_values_noiseless[temporal_index][best_es][best_idx]

    assert best_target_value == best_val
