...
class CBO(Root):
    def __init__(
        ...
        intervention_samples_noiseless: dict,
    ...
    def _update_bo_model(
        ...
        # prior_len = priors.InverseGamma.from_EV(2, 1.)
        # prior_sigma_f = priors.InverseGamma.from_EV( 2 /  (0.5* np.sqrt(n)), 1)
        # model.kern.lengthscale.set_prior(prior_len)
        # model.kern.variance.set_prior(prior_sigma_f)
